  ...
  const std::shared_ptr<nav2_costmap_2d::Costmap2DROS>)
{
  ...
  node_->get_parameter(plugin_name_ + ".stopped_linear_velocity", stopped_linear_);
  node_->get_parameter(plugin_name_ + ".stopped_angular_velocity", stopped_angular_);
  ...
  {
    throw std::runtime_error("PersistentGoalChecker hysteresis tolerances are invalid");
  }
}
...
bool PersistentGoalChecker::goalChanged(const geometry_msgs::msg::Pose & goal_pose) const
{
  if (!have_goal_) {
    return true;
  }
  ...
  return std::hypot(dx, dy) > goal_change_xy_ || std::abs(dyaw) > goal_change_yaw_;
}

void PersistentGoalChecker::startGoal(const geometry_msgs::msg::Pose & goal_pose)
{
  goal_ = goal_pose;
  have_goal_ = true;
  xy_latched_ = false;
  ...
  const geometry_msgs::msg::Twist & velocity)
{
  ...
  if (!yaw_latched_ && yaw_error <= yaw_enter_) {
    yaw_latched_ = true;
    RCLCPP_INFO(node_->get_logger(), "terminal yaw latched at %.3f rad", yaw_error);
  }
...
PLUGINLIB_EXPORT_CLASS(
  robonix_nav2_terminal::PersistentGoalChecker, nav2_core::GoalChecker)
